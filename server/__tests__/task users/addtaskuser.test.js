...
describe('adding task users', () => {
    test('user already requested for a task', async () => {
        ...
        expect(res.body.error).toBe("user already requested for the task");  // error => message ???
    });
    ...
    test('new entry by user for the task', async () => {
        ...
        expect(res.body.error).toBe("User availability successfully added");  // error => message ???
    });
});