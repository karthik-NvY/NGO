...
describe('User Update', () => {
    ...
    test('Return error if no data is being updated', async () => {
        ...
        const res = await request("http://localhost:8080").post('/user/update').send(packet).set('Authorization', `Bearer ${token}`);
    ...
    test('No user found with the provided ID', async () => {
        ...
        const res = await request("http://localhost:8080").post('/user/update').send(packet).set('Authorization', `Bearer ${newloginres.body.token}`);
